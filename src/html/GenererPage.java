...
public class GenererPage
{
	...
	{
		if ( choix == 1 )
			this.fichier = inter.getId() + "_" + inter.getNom() + "_" + inter.getPrenom() + ".txt";
		else
			this.fichier = module.getCode() + ".txt";
	...
	public static void genererPageInter ( Intervenant inter)
	{
		...
		try
		{
			...
			for ( int cptNbSemestres = 0; cptNbSemestres < 3; cptNbSemestres ++ )
				pw.print( creerTableauSemestres( cptNbSemestres, page.genererLstAffectations(inter.getModulesOuIntervient(), (cptNbSemestres + 1)) ) );
	...
	private String creerTableauRecapitulatif ( Intervenant inter )
	{
		...
		for ( int cptNbSemestres = 1; cptNbSemestres <= this.tabSemestre.length; cptNbSemestres++ )
		{
			res += tab + "<tr>\n";
	...
	private String creerHead( int type )
	{
		...
		res += "<link rel = 'stylesheet' href = \"./style.css\" media = 'all' type = 'text/css'>\n";
